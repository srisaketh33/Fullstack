...
async def create_shipment(shipment: ShipmentCreate):
    ...
    # Validate device id exists in telemetry (helps link pages correctly)
    device_id = shipment_data.get("device")
    try:
        has_data = device_collection.find_one({
            "$or": [
                {"Device Id": device_id},
                {"Device_ID": device_id},
                {"DeviceId": device_id}
    ...
    if not has_data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Device '{device_id}' has no telemetry. Choose a valid Device Id from Device Data page."
        )
