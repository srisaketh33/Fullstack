# backend/user.py
from fastapi import APIRouter, HTTPException, status, Depends, Response, Request, Form, Body
...
async def change_password(
    ...
    old = old_password or body.get("old_password") or body.get("oldPassword")
    new = new_password or body.get("new_password") or body.get("newPassword")

    if not old or not new:
        raise HTTPException(status_code=422, detail="old_password and new_password are required")

    doc = users_collection.find_one({"_id": ObjectId(current_user["id"])})
    if not doc or not verify_password(old, doc.get("hashed_password", "")):
        raise HTTPException(status_code=400, detail="Incorrect old password")

    new_hashed = hash_password(new)
    users_collection.update_one({"_id": ObjectId(current_user["id"])}, {"$set": {"hashed_password": new_hashed}})
